...
class GuayaquilSoapWrapper
{
    ...
    function getSoapClient($laximoOem = true)
    {
        ...
        if ($laximoOem) {
            ...
            $options['location'] = ($this->authMethod == 'certificate' ? 'https' : 'http').'://oem-api.yqservice.eu/ec.Kito.WebCatalog/services/Catalog.CatalogHttpSoap11Endpoint/';
        } else {
            $options['uri'] = 'http://Aftermarket.Kito.ec';
            $options['location'] = ($this->authMethod == 'certificate' ? 'https' : 'http').'://oem-api.yqservice.eu/ec.Kito.Aftermarket/services/Catalog.CatalogHttpSoap11Endpoint/';
        }
