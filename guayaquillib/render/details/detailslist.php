...
class GuayaquilDetailsList extends GuayaquilTemplate
{
    ...
    function Draw($catalog, $details, $unitlink, $buyButton)
    {
        ...
        if ($this->group_by_filter) {
            ...
            foreach ($searched as $detail) {
                $html .= $this->DrawItem($catalog, $detail, $buyButton);
                $this->rowno++;
            }
        } else {
            foreach ($details as $detail) {
    ...
    function DrawItem($catalog, $detail, $buyButton)
    {
        $html        = $this->DrawDetailRow($detail, $this->columns, false, $catalog, $buyButton);
    ...
    function DrawDetailRow($detail, $columns, $isArray, $catalog, $buyButton)
    {
        $html = '<tr';
        ...
        foreach ($this->basic_columns as $column => $visibility)
            $html .= $this->DrawDetailCell($detail, strtolower($column), $visibility, $isArray, $buyButton);
    ...
    function DrawDetailCell($detail, $column, $visibility, $isArray, $buyButton)
    {

        $html = '<td name="c_' . $column . '"';
        if ((($visibility & 1) == 0) && ($column !== 'oem')) {
        ...
        $html .= $this->DrawDetailCellValue($detail, $column, $visibility, $isArray, $buyButton);
    ...
    function DrawDetailCellValue($detail, $column, $visibility, $isArray, $buyButton)
    {
        switch ($column) {
            case 'pnc':
            ...
            case 'oem':
				$article = $this->getProperty($detail, 'oem', $isArray);
				$link = '/catalog/number?q='.$article;
				if ($buyButton != 1) {
				$html = '<a style="color:00497c" href="'.$link.'" target="_blank">';
				$html .= substr($article, 0, 1);
				$html .= "******";
				$html .= substr($article, -1);
				$html .= '</a>';
				}
				else {
				$html = '<a style="color:00497c" href="'.$link.'" target="_blank">';
            ...
            case 'name':
				...
				$article = $this->getProperty($detail, 'oem', $isArray);
				$newlink = '/catalog/number?q='.$article;
				if ((string)$article == '') {
					$newlink = "javascript:void(0)";
				}
                $html = '<a href="'.$newlink.'" target="_blank">';
                $name = $this->getProperty($detail, 'name', $isArray);
                if (!strlen((string)$name)) {
                    $name = 'Наименование не указано';
                }
    ...
    {
        return '<th id="c_' . $column . '"' . ((($visibility & 1) == 0) && ($column !== 'oem') ? ' style="display:none;"' : '') . '>' . $this->DrawHeaderCellValue($column, $visibility) . '</th>';
    }
